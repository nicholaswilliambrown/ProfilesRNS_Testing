...
Declare @oldDB varchar(255) = '$(UpgradeDatabaseName)'
...
select query from #updateQueries order by TableName, query

declare @numberOfQueries as int
select @numberOfQueries = count(*) from #updateQueries
...
if @numberOfQueries <> 0 begin raiserror('Data Not in Sync', 16, 1) end